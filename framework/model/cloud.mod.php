...
define('CLOUD_GATEWAY_URL', 'http://cloud.zhangshuoyin.cn/gateway.php');
define('CLOUD_GATEWAY_URL_NORMAL', 'http://cloud.zhangshuoyin.cn/gateway.php');
...
function cloud_request($url, $post = '', $extra = array() , $timeout = 60) {
    ...
    if (!empty($_W['setting']['cloudip']['ip']) && empty($extra['ip'])) {
        ...
        //$extra['ip'] = "cloud.zhangshuoyin.cn";
...
function cloud_build() {
    ...
    $dat = cloud_request('http://cloud.zhangshuoyin.cn/gateway.php', $pars);
...
function cloud_download($path, $type = '') {
    ...
    $dat = cloud_request('http://cloud.zhangshuoyin.cn/gateway.php', $pars, $headers, 300);
    if (is_error($dat)) {
...
function cloud_sms_send($mobile, $content, $postdata = array()) {
    ...
    $response = cloud_request('http://cloud.zhangshuoyin.cn/gateway.php', $pars);
...
function cloud_cron_create($cron) {
    ...
    $result = cloud_request('http://cloud.zhangshuoyin.cn/gateway.php', $pars);
...
function cloud_auth_url($forward, $data = array()) {
    ...
    $auth_url = 'http://cloud.zhangshuoyin.cn/gateway.php?__auth=' . $query;
    return $auth_url;
}
...
function cloud_flow_master_post($flow_master) {
    ...
    $dat = cloud_request('http://cloud.zhangshuoyin.cn/gateway.php', $pars, array() , 300);
    if (is_error($dat)) {
        return error(-1, '网络存在错误， 请稍后重试。' . $dat['message']);
    }
