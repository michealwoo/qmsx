...
<script type='text/html' id='tpl_goods_list'>    
	...
	<div class="fui-goods-item" data-goodsid="<%g.id%>" data-type="<%g.type%>">		
		...
		<!-- <div class="salez" style="background-image: url('<?php  echo tomedia($_W['shopset']['shop']['saleout'])?>'); "></div> -->		<!--<a <%if g.bargain>0%>href="<?php  echo mobileUrl('bargain/detail')?>&id=<%g.bargain%>"<%else%>href="<?php  echo mobileUrl('goods/detail')?>&id=<%g.id%>"<%/if%>>-->		 <?php  if(!empty($_W['shopset']['shop']['saleout'])) { ?>		 <!--<%if g.total<=0%><div class="salez" style="background-image: url('<?php  echo tomedia($_W['shopset']['shop']['saleout'])?>'); "></div><%/if%>-->		 <?php  } else { ?>		 <!--<%if g.total<=0%><div class="salez" style="background-image: url('<?php  echo '../addons/ewei_shopv2/static/images/shouqin.png'?>'); "></div><%/if%>-->		 <?php  } ?>		<!--</a>-->	
		</div>    
	<%/each%>
</script>
