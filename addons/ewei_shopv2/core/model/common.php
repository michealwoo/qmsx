...
class Common_EweiShopV2Model 
{
	...
	//根据会员信息获取折扣后商品价格
	public function getmarketprice($openid,$goodsprice,$discounts)
	{
		if(empty($openid) || empty($discounts)||empty($goodsprice)){
		   return false;
		}

		 //会员折扣
        $level = m('member')->getLevel($openid);
        $discount = json_decode($discounts,true);

        if(!empty($discount))
        {
            $member_zhongji = pdo_get('ewei_shop_member_level', array('id' => 1));
            $member_gaoji = pdo_get('ewei_shop_member_level', array('id' => 2));
            $member_vip = pdo_get('ewei_shop_member_level', array('id' => 3));
        	switch ($level['levelname'])
            {
                case '中级会员':
                    if(empty($discount['level1'])){
                       $marketprice = bcmul(floatval($member_zhongji['discount'])*0.1,floatval($goodsprice),3);
                    }else{
                       $marketprice = bcmul(floatval($discount['level1'])*0.1,floatval($goodsprice),3);
                    }
                    break;
                case '高级会员':
                    if(empty($discount['level2'])){
                       $marketprice = bcmul(floatval($member_gaoji['discount'])*0.1,floatval($goodsprice),3);
                    }else{
                       $marketprice = bcmul(floatval($discount['level2'])*0.1,floatval($goodsprice),3);
                    }
                    break;
                case 'VIP会员':
                    if(empty($discount['level3'])){
                       $marketprice = bcmul(floatval($member_vip['discount'])*0.1,floatval($goodsprice),3);
                    }else{
                       $marketprice = bcmul(floatval($discount['level3'])*0.1,floatval($goodsprice),3);
                    }
                    break;
                default:
                    if(empty($discount['default']))
                    {
                       ...
                       $marketprice = bcmul(floatval($set['shop']['leveldiscount'])*0.1,floatval($goodsprice),3);
                    }else{
                       $marketprice = bcmul(floatval($discount['default'])*0.1,floatval($goodsprice),3);
                    }
                    break;
            }
