...
class Goods_EweiShopV2Model 
{
	...
	public function getList($args = array( )) 
	{
		...
		if( !$random ) 
		{
			$sql = "SELECT id,title,subtitle,thumb,thumb_url" . $officsql . ",marketprice,productprice,minprice,maxprice,discounts,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video,bargain,hascommission,nocommission,commission,commission1_rate,commission1_pay,presellprice\r\n            FROM " . tablename("ewei_shop_goods") . " where  " . $condition . " ORDER BY " . $order . " " . $orderby . " LIMIT " . ($page - 1) * $pagesize . "," . $pagesize;
			$total = pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_goods") . " where  " . $condition . " ", $params);
		}
		else 
		{
			$sql = "SELECT id,title,thumb,thumb_url" . $officsql . ",marketprice,productprice,minprice,maxprice,discounts,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,bargain,hascommission,nocommission,commission,commission1_rate,commission1_pay,presellprice\r\n            FROM " . tablename("ewei_shop_goods") . " where  " . $condition . " ORDER BY rand() LIMIT " . $pagesize;
			$total = $pagesize;
		}
