...
class System_EweiShopV2Model {
    ...
    public function init() {
        ...
        $arr["quoteTotal"]=$this->getQuoteTotal();
        $arr["bidTotal"]=$this->getBidTotal();
        if (!$this->merch) {
    ...
    protected function getQuoteTotal() {
        global $_W;
        $sql = " SELECT count(*) as total FROM ".tablename('quote')." WHERE isread=0";
        return pdo_fetch($sql)['total'];
    }
    //获取竞价未读信息
    protected function getBidTotal() {
        global $_W;
        $sql = " SELECT count(*) as total FROM ".tablename('bid')." WHERE isread=0";
        return pdo_fetch($sql)['total'];
    }
