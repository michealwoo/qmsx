...
class Notice_EweiShopV2Model 
{
	...
	//供货商分单信息推送
    public function sendSupplierMessage($openid,$goods_list=array(),$supplier_name)
	{
		global $_W;

		if( empty($openid)|| empty($goods_list))
		{
			return false;
		}
		//$member = m("member")->getMember($openid);
		//$url = $this->getUrl("order/detail", array( "id" => 1 ));
        ...
        foreach ($goods_list as &$val) 
        {
           $goods_str .="\n商品名称:".$val['goods_name'].
				        //"\n品类:".$val['cname'].
				        "\n商品个数:".$val['_total'].
				        "\n订单总价:".$val['total_price'].
				        "\n分单信息:".$val['fenbao'].
				        "\n-------------------------------";
        }  

        $message = $supplier_name.",您有新的分单信息:" .$goods_str. "\n通知时间:" .date('Y-m-d H:i:s',time()). "\n\n如有疑问请联系在线客服。\n";
        
        return m("message")->sendTexts($openid, $message, '' , null);
	}
	public function sendDeliveryMessage($openid, $order, $name)
	{
		global $_W;
		
		if( empty($openid)|| empty($order)||empty($name) )
		{
			return false;
		}
        ...
        $message = $name.",您有新的配送任务:\n订单编号:".$order['ordersn']."\n配送人:" . $name . "\n收货人:" .$address['realname']. " \n联系方式:".$address['mobile']."\n配送地址: " . trim($address['province']).trim($address['city']).trim($address['area']).trim($address['address']) .  "\n配送时间:" .date('Y-m-d H:i:s',time()). "\n\n如有疑问请联系在线客服。\n\n<a href='" . $url . "'>点击查看详情</a>";

        m("message")->sendTexts($openid, $message, '' , null);
	}
