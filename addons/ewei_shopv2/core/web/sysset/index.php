...
class Index_EweiShopV2Page extends WebPage 
{
	...
	public function ordersetting()
	{
        ...
        {
	        $arr = array();
            foreach ($_GPC['start'] as $key => $val) 
			{
				$arr[] = $val.'-'.$_GPC['end'][$key];
			}

			if( empty($data) )
			{
				pdo_insert("order_date_setting", array(
					'id'=>1,
			...
			{
			   pdo_update("order_date_setting", array( 
				   	"date" =>json_encode( $arr ),
				   	'isshow'=>$_GPC['isshow'],
				   	'update_time'=>time(),
				   	'tips'=>$_GPC['tips']
        ...
        $isshow = $data[0]['isshow'];
       include($this->template("sysset/ordersetting"));
	}
