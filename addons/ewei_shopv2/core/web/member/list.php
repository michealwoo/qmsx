...
class List_EweiShopV2Page extends WebPage 
{
	public function main() 
	{
		...
		$sql = "select * ,dm.openid from " . tablename("ewei_shop_member") . " dm " . $join . " where 1 " . $condition . "  ORDER BY dm.id DESC,dm.lastplaceordertime DESC ";
		...
		foreach( $list as &$row ) 
		{
		    ...
		    $placeordertime = strtotime(date('Y-m-d',$row['lastplaceordertime']));
            $row['placeorderdays'] = round(($today-$placeordertime)/3600/24);
