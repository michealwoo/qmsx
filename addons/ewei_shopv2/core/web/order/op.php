	...
	public function send() 
	{
		...
		if( $_W["ispost"] )
		{
			if( $item["city_express_state"] == 0 ) 
			{
				...
				$time = time();
				$data = array( "sendtype" => (0 < $item["sendtype"] ? $item["sendtype"] : intval($_GPC["sendtype"])), "express" => trim($_GPC["express"]), "expresscom" => trim($_GPC["expresscom"]), "expresssn" => trim($_GPC["expresssn"]), "sendtime" => $time ,'deliveryman_id'=> $_GPC['deliveryman_id']);
				...
				else 
				{
					$data["status"] = 2;
					$data["refundid"] = 0;
					$data["deliveryman_id"] = $_GPC['deliveryman_id'];
					pdo_update("ewei_shop_order", $data, array( "id" => $item["id"], "uniacid" => $_W["uniacid"] ));
				}
			}
			else {
			...
			$info = pdo_get('delivery_staff',array('id'=>$_GPC['deliveryman_id']),array('name','mid'));
		    $openid = pdo_get('ewei_shop_member',array('id'=>$info['mid']),array('openid'))['openid'];
         
            m("notice")->sendDeliveryMessage($openid,$item,$info['name']);
		...
		$delivery = pdo_fetchall("select id,name from ".tablename("delivery_staff").' where workstate=1');
