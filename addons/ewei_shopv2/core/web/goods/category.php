...
class Category_EweiShopV2Page extends WebPage 
{
	...
	protected function post() 
	{
		...
		if( $_W["ispost"] ) 
		{
			// if( empty($_GPC['supplier_id']) || $_GPC['supplier_id']==0 ) 
			// {
			// 	$this->message("请选择供货商", webUrl("category/add"), "error");
			// }
			$data = array( "uniacid" => $_W["uniacid"], "name" => trim($_GPC["catename"]), "enabled" => intval($_GPC["enabled"]), "displayorder" => intval($_GPC["displayorder"]), "isrecommand" => intval($_GPC["isrecommand"]), "ishome" => intval($_GPC["ishome"]), "description" => $_GPC["description"], "supplier_id"=>$_GPC['supplier_id'], "parentid" => intval($parentid), "thumb" => save_media($_GPC["thumb"]), "advimg" => save_media($_GPC["advimg"]), "advurl" => trim($_GPC["advurl"]), "level" => $level );
			if( !empty($id) ) 
			{
				unset($data["parentid"]);
	    ...
	    $supplier = pdo_fetchall("select o.id,concat(o.company,o.name) as name from ".tablename('supplier').' o ');
		include($this->template());
	}
