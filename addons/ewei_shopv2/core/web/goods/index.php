	...
	public function main($goodsfrom = "sale") 
	{
		...
		if( !empty($total) ) 
		{
			...
			foreach( $list as $key => &$value ) 
			{
				...
				if(empty($value["unit"])){
                    $value["title"] = $value["title"].'/单位'.$value["unit"];
				}
				else {
					$value["title"] = $value["title"].'/'.$value["unit"];
				}
