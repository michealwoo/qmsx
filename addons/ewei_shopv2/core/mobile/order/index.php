...
class Index_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function get_list() 
	{
		...
		{
			$param = array( );
			$delivery_man = pdo_get('delivery_staff',array('id'=>$row['deliveryman_id']),array('name','phone'));
			$row['delivery_man_name'] = $delivery_man['name'];
			$row['delivery_man_phone'] = $delivery_man['phone'];
			if( $row["isparent"] == 1 ) 
			{
	...
	public function detail() 
	{
		...
		$deliveryman = pdo_get('delivery_staff',array('id'=>$order['deliveryman_id']),array('name','phone'));
		//print_r($order);exit;
