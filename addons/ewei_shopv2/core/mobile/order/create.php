...
class Create_EweiShopV2Page extends MobileLoginPage 
{
	...
	{   
		...
		//判断下单时间
		$data = pdo_fetchall("select * from " . tablename("order_date_setting"));
        $date = array();

        if(!empty($data))
        {   
            foreach ($data as $key => $val) {
            	$arr = json_decode($val['date']);
            }
		...
		if( !$packageid ) 
		{
			...
			if( empty($id) ) 
			{
				...
				foreach( $goods as $k => &$v ) 
				{
					...
					//会员折扣
                    $v['marketprice'] = m('common')->getmarketprice($_W['openid'],$v['marketprice'],$v['discounts']);
				}
			...
			else 
			{
				...
				{   					
		            ...
		            //会员折扣
		            $data['marketprice'] = m('common')->getmarketprice($_W['openid'],$data['marketprice'],$data['discounts']);
	...
	public function submit() 
	{
		...
		//会员写入最后下单时间
		pdo_update('ewei_shop_member',array('lastplaceordertime'=>time()), array('openid' => $_W['openid']));
